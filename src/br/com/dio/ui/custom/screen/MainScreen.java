...
public class MainScreen {
    ...
    public void buildMainScreen() {
        ...
        for (int r = 0; r < 9; r += 3) {
            for (int c = 0; c < 9; c += 3) {
                JPanel sector = generateSection(c, r);
    ...
    private JPanel generateSection(final int initCol, final int initRow) {
        List<NumberText> fields = new ArrayList<>();
        int endCol = initCol + 2;
        int endRow = initRow + 2;

        for (int row = initRow; row <= endRow; row++) {
            for (int col = initCol; col <= endCol; col++) {
                Space space = boardService.getSpaces().get(col).get(row);
                NumberText numberText = new NumberText(space, boardService, col, row);
